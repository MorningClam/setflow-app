/**
 * Renders the correct bottom navigation bar based on user role and tier.
 * @param {string} role - The user's primary role ('musician', 'venue', 'promoter').
 * @param {string} tier - The user's subscription tier (e.g., 'free', 'pro').
 * @param {string} activePage - The filename of the current page (e.g., 'setflow-inbox.html').
 */
function renderBottomNav(role, tier = 'free', activePage = '') {
    const navContainer = document.getElementById('bottom-nav-container');
    if (!navContainer) {
        console.warn(`Bottom nav container '#bottom-nav-container' not found on this page. Skipping nav render.`);
        return;
    }
    ...
    if (role === 'musician') {
        ...
        // Add "Applied" only for Pro users, matching musician-dashboard.html logic
        if (tier !== 'free') {
            navHtml += `
        ...
        `;
    } else if (role === 'venue' || role === 'promoter') {
        const dashboardLink = role === 'venue' ? 'setflow-venue-dashboard.html' : 'setflow-promoter-dashboard.html';
        const settingsLink = role === 'venue' ? 'setflow-venue-settings.html' : 'setflow-promoter-settings.html';
        
        navHtml = `
            <a href="${dashboardLink}" class="bottom-nav-link ${isActive(dashboardLink)}" ${isCurrent(dashboardLink)} aria-label="Dashboard">
            ...
            <a href="setflow-inbox.html" class="bottom-nav-link ${isActive('setflow-inbox.html')}" ${isCurrent('setflow-inbox.html')} aria-label="Inbox">
                <svg aria-hidden="true" class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2v10z" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                <span class="text-xs font-medium">Inbox</span>
            </a>
